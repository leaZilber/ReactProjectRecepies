...
import type { Recipe, CategoryModel } from "./models"
...
export const AllRecipes = () => {
  ...
  const difficultyValues = [1, 2, 3, 4]
  ...
  const fetchRecipes = async () => {
    try {
      ...
      const processedData = data.map((recipe: Recipe) => {
        if (recipe.Categoryid && typeof recipe.Categoryid === 'object' && 'Id' in recipe.Categoryid) {
          return {
            ...recipe,
            Categoryid: recipe.Categoryid.Id
          };
        }
        return recipe;
      });
      ...
      processedData.forEach((recipe: Recipe) => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== null) {
      filtered = filtered.filter((recipe) => {
        const categoryId = typeof recipe.Categoryid === 'object' && recipe.Categoryid !== null
  ...
  const getCategoryName = (categoryId: number | CategoryModel): string => {
        ...
        <FormGroup sx={{ px: 1 }}>
          {difficultyValues.map((difficultyValue) => (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={selectedDifficulties.includes(difficultyValue)}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setSelectedDifficulties([...selectedDifficulties, difficultyValue])
                    } else {
                      setSelectedDifficulties(selectedDifficulties.filter((d) => d !== difficultyValue))
                    }
                  }}
                  sx={{
                    color: "#BFAF9B",
              ...
              label={difficultyValues[difficultyValue]}
              sx={{ direction: "rtl" }}
            />
          ))}
        </FormGroup>
