...
export const AddRecipeForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    // UserId: 1, // ברירת מחדל אם אין משתמש מחובר
    Duration: "",
    Difficulty: "",
    Categoryid: "",
  ...
  const mapDifficulty = (num: number): DifficultyType => {
    const mapping: Record<number, DifficultyType> = {
      1: "קל",
      2: "בינוני",
      3: "מתקדם",
      4: "מורכב",
    }
    return mapping[num] || ("" as DifficultyType)
  }
  ...
  useEffect(() => {
    if (editMode && recipeToEdit) {
      setFormData({
        ...
        Difficulty: mapDifficulty(Number(recipeToEdit.Difficulty)) || "",
        Categoryid: recipeToEdit.Categoryid ? recipeToEdit.Categoryid.Id.toString() : "",
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          if (categoriesArray && categoriesArray.length > 0) {
            ...
            console.error("מבנה הנתונים לא תקין", data)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!selectedCategory) {
        throw new Error(`קטגוריה לא נמצאה (ID: ${formData.Categoryid})`)
      }
      ...
      const recipeData = {
        ...
        UserId: editMode && recipeToEdit ? recipeToEdit.UserId : currentUserId,
      ...
      setSnackbar({
        open: true,
        message: error.response?.data || error.message || "אירעה שגיאה בשמירת המתכון",
        severity: "error",
      })
    } finally {
      setLoading(false)
...
// const unitTypes: UnitType[] = ["גרם", "כוסות", "כפיות", "חבילות", "יחידות", "כפות"]
// const difficultyTypes: DifficultyType[] = ["קל", "בינוני", "מתקדם", "מורכב"]
...
//       // If in edit mode, add the original ID
//       if (editMode && recipeToEdit) {
//         recipeData.UserId = recipeToEdit.Id
//       }
