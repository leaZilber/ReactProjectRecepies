...
// import { CategoryModel, DifficultyType, Instruc, Product, Recipe, UnitType } from "./models"
...
//         Categoryid: recipeToEdit.Categoryid?.Id ?? 1, 
...
export const AddRecipeForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    Duration: "",
    Difficulty: "1",
    Categoryid: 1, // שימוש במספר ולא באובייקט
    Ingredients: [] as Product[],
    Instructions: [] as Instruc[],
  })
  ...
  useEffect(() => {
    if (editMode && recipeToEdit) {
      // שינוי: שמירת מזהה הקטגוריה כמספר ולא כאובייקט
      const categoryId =
      ...
      setFormData({
        ...
        Duration: recipeToEdit.Duration ? recipeToEdit.Duration.toString() : "",
        Difficulty: recipeToEdit.Difficulty ? recipeToEdit.Difficulty.toString() : "1",
        Categoryid: categoryId, // שמירת מזהה הקטגוריה כמספר
        Ingredients: recipeToEdit.Ingridents || [],
        Instructions: recipeToEdit.Instructions || [],
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recipeData = {
        ...
        Description: formData.Description,
        UserId: editMode && recipeToEdit ? recipeToEdit.UserId : getCurrentUser().id,
        Categoryid: categoryId, // שליחת מזהה הקטגוריה כמספר
        Ingridents: Ingredients,
        Instructions: Instructions,
      }
